...
{
    public partial class Form_caixa1 : MaterialForm
    {
        ...
        public Form_caixa1(ProdutoDTO produto)
        {
            InitializeComponent();
            ...
            dgprodutos.RowsDefaultCellStyle.Font = new Font("Arial", 10);
            dgprodutos.RowsDefaultCellStyle.Format = "N2";
        ...
        private void Form_caixa1_Load(object sender, EventArgs e)
        {
            // valida caixa aberto
            var stCaixa = _movcaixa.StatusMovCaixa();

            if (stCaixa == 3)
            {

                MsgBox.Show("Caixa está fechado", "Status", MsgBox.Buttons.OK, MsgBox.Icon.Error);
        ...
        private void lblcompra_TextChanged(object sender, EventArgs e)
        {
            AtualizaValor();
        }
        ...
        public void carregaGridCaixa()
        {
            ...
            dgCaixa.Columns[3].HeaderText = "Total R$";
            dgCaixa.Columns[0].Width = 250;
            dgCaixa.AutoResizeColumn(1);
            dgCaixa.Columns[3].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
        }
        private void AddProduto(ProdutoDTO produto)
        {
            ...
            itemPedido.ValorTotal = produto.preco;
            ...
            else
            {
                ...
                for (int i = 0; i < NovoPedido.Count; i++)
                {
                    ...
                    {
                        NovoPedido[i].Qtd++;
                        NovoPedido[i].ValorTotal += produto.preco;
                    }
            ...
            lblcompra.Text = Math.Round(NovoPedido.Sum(x => x.ValorTotal), 2).ToString("N");
            carregaGridCaixa();
        ...
        {
            var respImp = _ticket.ImprimirTicketMP4200(ticket.Id);
            if (respImp == "OK")
            {
                MsgBox.Show("Reimpressão Concluida!", "Sucesso!", MsgBox.Buttons.OK, MsgBox.Icon.Info);
                this.Close();
            }
            else
            {
